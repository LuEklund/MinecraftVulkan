...
MvSwapChain::MvSwapChain(MvDevice&deviceRef, VkExtent2D extent)
    : device{deviceRef}, windowExtent{extent} {
      init();
}

MvSwapChain::MvSwapChain(MvDevice &deviceRef, VkExtent2D windowExtent, std::shared_ptr<MvSwapChain> previous)
: device{deviceRef}
...
MvSwapChain::~MvSwapChain() {
  ...
  for (auto framebuffer : swapChainFramebuffersSky) {
    vkDestroyFramebuffer(device.GetDevice(), framebuffer, nullptr);
  }
...
VkResult MvSwapChain::acquireNextImage(uint32_t *imageIndex) {
  vkWaitForFences(
      device.GetDevice(),
      1,
...
VkResult MvSwapChain::submitCommandBuffers(
...
void MvSwapChain::createSwapChain()
{
...
void MvSwapChain::createImageViews() {
...
void MvSwapChain::createRenderPassSkyBox() {
  VkAttachmentDescription depthAttachment{};
  depthAttachment.format = findDepthFormat();
  depthAttachment.samples = VK_SAMPLE_COUNT_1_BIT;
  depthAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
...
void MvSwapChain::createFramebuffers() {
  ...
  // swapChainFramebuffersSky.resize(GetimageCount());
  // for (size_t i = 0; i < GetimageCount(); i++) {
  //   std::array<VkImageView, 2> attachments = {swapChainImageViews[i], depthImageViews[i]};
  //
  //   VkExtent2D swapChainExtent = GetSwapChainExtent();
  //   VkFramebufferCreateInfo framebufferInfo = {};
  //   framebufferInfo.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
  //   framebufferInfo.renderPass = renderPass; //TODO : seperate renderpass?
  //   framebufferInfo.attachmentCount = static_cast<uint32_t>(attachments.size());
  //   framebufferInfo.pAttachments = attachments.data();
  //   framebufferInfo.width = swapChainExtent.width;
  //   framebufferInfo.height = swapChainExtent.height;
  ...
  //           &framebufferInfo,
  //           nullptr,
  //           &swapChainFramebuffersSky[i]) != VK_SUCCESS) {
  //     throw std::runtime_error("failed to create framebuffer!");
  //           }
  // }
}
...
VkSurfaceFormatKHR MvSwapChain::chooseSwapSurfaceFormat(
...
VkPresentModeKHR MvSwapChain::chooseSwapPresentMode(
...
VkExtent2D MvSwapChain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
...
VkFormat MvSwapChain::findDepthFormat() {
