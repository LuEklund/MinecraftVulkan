	...
	: m_Device(eDevice)
{
	...
	else if (Type == PipelineType::UI) {
		createUIPipeline(ConfigInfo);
	}
	else
...
void MvPipeline::createUIPipeline(const PipelineConfigInfo& configInfo)
{
	assert(
		configInfo.pipelineLayout != VK_NULL_HANDLE &&
		"Cannot create graphics pipeline: no pipelineLayout provided in configInfo");
	...
	std::vector<uint32_t> vertexShaderCode;
	std::vector<uint32_t> fragShaderCode;
	if (!CompileShader("shaders/UI.vert", Vertex, vertexShaderCode))
	{
		throw std::runtime_error("Failed to compile vertex shader");
	}

	if (!CompileShader("shaders/UI.frag", Fragment, fragShaderCode))
	{
		throw std::runtime_error("Failed to compile fragment shader");
	}
	...
	auto bindingDescriptions = MvUIModel::UIVertex::getBindingDescriptions();
	auto attributeDescriptions = MvUIModel::UIVertex::getAttributeDescriptions();
